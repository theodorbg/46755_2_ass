...
def plot_cumulative_distribution_func(scenario_profits, expected_profit, scheme_type):
    ...
    plt.axvline(x=expected_profit, color='r', linestyle='--', label=f'Expected Profit: {(expected_profit)/1e3:.1e} k EUR')
    plt.xlabel('Profit (EUR)', fontsize=22)
    plt.ylabel('Cumulative Probability', fontsize=22)
    plt.title(f'Cumulative Distribution of Profit - {scheme_type} Balancing Scheme', fontsize=22)
    plt.grid(alpha=0.3)
...
def compare_offers(optimal_offers_one_price, optimal_offers_two_price):
    ...
    plt.ylabel('Offer Quantity (MW)', fontsize=22)
    plt.title('Comparison of Optimal Day-Ahead Offers', fontsize=16)
    plt.grid(alpha=0.3)
...
def compare_all_strategies(optimal_offers_one_price, optimal_offers_two_price, ew_optimal_offers):
    ...
    plt.xlabel('Hour', fontsize=22)
    plt.ylabel('Offer Quantity (MW)', fontsize=22)
    plt.title('Comparison of Bidding Strategies', fontsize=22)
    plt.grid(alpha=0.3)
    plt.legend(fontsize=22)
    plt.xticks(fontsize=22)
    plt.yticks(fontsize=22)
